...
from datasets.not_used.in_memory_dataset import InMemoryDataset
from models.emg_key_press_classifier import EmgKeyPressClassifier
from svm_trains import train_lopo_svm
from train_utils import get_optimizer, train, get_sweep_config, init_sweep, start_sweep, load_checkpoint, \
    evaluate
...
def get_command_line_arguments(parser):
    ...
    parser.add_argument("--app-name", type=str,
                        default=f"in_memory_train_on_all_but_P1_eval_on_him")

    parser.add_argument('--run-or-sweep', type=str, default='sweep',
                        choices=['run', 'sweep'],
...
def sweep_train(sweep_id, args, config=None):
    with wandb.init(config=config):
        ...
        train_on_all_but_one_eval_on_him(cli_args, eval_participant=Participant.P1)
...
@torch.no_grad()
def plot_lopo_confusion_matrix(args):
    from sklearn.metrics import ConfusionMatrixDisplay
    import matplotlib.pyplot as plt
    eval_participant = Participant.P1
    eval_pt = [(eval_participant, t) for t in DayT1T2]
    eval_set = InMemoryDataset(participants=list(set([p[0] for p in eval_pt])))
    ...
    eval_accuracy, eval_loss, sk_acc, sk_f1, confusion_matrix, balanced_score = evaluate(model, eval_loader)
    logger.info(f'acc = {eval_accuracy}, loss = {eval_loss} sk f1 = {sk_f1}'
                f' sk acc = {sk_acc} balanced score {balanced_score}')

    ConfusionMatrixDisplay(confusion_matrix, display_labels=[k.value for k in KeyPress if k != KeyPress.SPACE]).plot()
    plt.show()

    logger.info('train using svm')
    acc, f1, cm = train_lopo_svm(args, eval_participant=Participant.P1)

    logger.info(f'SVM results acc {acc} f1 {f1}')
    ConfusionMatrixDisplay(cm, display_labels=[k.value for k in KeyPress if k != KeyPress.SPACE]).plot()
    plt.show()
...
def train_base(args, train_pt_pairs: list[tuple[Participant, DayT1T2]],
    ...
    train_set = InMemoryDataset(participants=list(set([p[0] for p in train_pt_pairs])), permute_channels=True)
    eval_set = InMemoryDataset(participants=list(set([p[0] for p in eval_pt_pairs])), permute_channels=False)
...
if __name__ == '__main__':
    ...
    if cli_args.run_or_sweep == 'run':
        ...
        # train_on_all_but_one_eval_on_him(cli_args, eval_participant=Participant.P1)
        plot_lopo_confusion_matrix(cli_args)
    else:
