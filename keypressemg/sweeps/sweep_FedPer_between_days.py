...
import keypressemg.fl_trainers.trainer_FedPer_between_days
...
def get_command_line_arguments(parser):
    ...
    parser.add_argument("--app-name", type=str, default=f"sweep_FedPer_between_days")
...
if __name__ == '__main__':
    argument_parser = argparse.ArgumentParser(description="Sweep FedPer Between Days Train Key Press EMG MLP")
    ...
    sweep_configuration = {
        "name": "sweep_FedPer_between_days_key_press_emg",
        "method": "grid",
        "metric": {"goal": "maximize", "name": "eval_acc"},
        "parameters": {
            "lr": {"values": [0.1]},
            "seed": {"values": [42, 43, 44]},
            "inner_steps": {"values": [1, 10]},
            "wd": {"values": [0.0001]},
            "num_steps": {"values": [600]},
            "num_client_agg": {"values": [10]},
            "num_personalization_layers": {"values": [1]},
        },
    }
    sweep(sweep_config=sweep_configuration, args=cli_args,
          train_fn=keypressemg.fl_trainers.trainer_FedPer_between_days.train)
