...
def eval_model(args, global_model, client_ids, clients_personalization_nets):
    ...
    for i, client_id in enumerate(client_ids):
        ...
        for time in range(1, 3):
            ...
            _, eval_set = get_same_split_day_datasets(Path(args.data_path), p, t, scale=True, shuffle=False)
            ...
            acc_dict[f"P{client_id}_T{time}"] = eval_accuracy
            loss_dict[f"P{client_id}_T{time}"] = running_loss
            acc_score_dict[f"P{client_id}_T{time}"] = acc_score
            f1s_dict[f"P{client_id}_T{time}"] = f1
...
def train(args):
    ...
    device = get_device(cuda=int(args.gpus) >= 0, gpus=args.gpus)
    clients_personalization_nets: dict[int: nn.Module] = {cid: get_personalization_net(args) for cid in client_ids}
    ...
    for step in step_iter:
        ...
        time = np.random.randint(low=1, high=3, size=args.num_client_agg)
        ...
        for j, c_id in enumerate(client_ids_step):
            ...
            t = DayT1T2[f"T{time[j]}"]

            train_set, _ = get_same_split_day_datasets(Path(args.data_path), p, t, scale=True, shuffle=False)
            ...
            for i in range(args.inner_steps):
                ...
                step_iter.set_description(
                    f"Step: {step + 1}, client: {c_id}_{time[j]}, Inner Step: {i}, Loss: {loss.item()}"
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--app-name", type=str, default=f"train_FedPer_mlp_split_day", )
    ...
    exp_name = f'FedPer_split-day_seed_{args.seed}_wd_{args.wd}_' \
